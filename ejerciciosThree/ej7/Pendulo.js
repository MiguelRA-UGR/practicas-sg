...
class Escalera extends THREE.Object3D {
	...
	createGUI(gui) {
		...
		folder.add(this.guiControls, 'pen_sup_rotate', -Math.PI/4, Math.PI/4, 0.1).name('Rotar P1: ').listen();
		folder.add(this.guiControls, 'rojo_size', 5, 10, 0.1).name('SizeY P1: ').listen();

		folder2.add(this.guiControls, 'ver').name('Ver Objeto: ');
		folder2.add(this.guiControls, 'pen_inf_rotate', -Math.PI/4, Math.PI/4, 0.1).name('Rotar P2: ').listen();
	...
	update() {
		this.parte2.scale.y = this.guiControls.rojo_size;
		this.Pendulo.rotation.z = this.guiControls.pen_sup_rotate;
		this.parte3.position.y = -2-this.h*this.guiControls.rojo_size;
		this.Pendulo.visible = this.guiControls.ver;

		this.parte4.scale.y = this.guiControls.pendulo2_size;
		this.pendulo_inf.rotation.z = this.guiControls.pen_inf_rotate;

		this.pendulo_inf.position.y = this.pendulo_offset - ((this.guiControls.posicion * this.guiControls.rojo_size) * 0.01);
